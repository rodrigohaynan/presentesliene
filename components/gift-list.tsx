...
type GiftItem = { id: string; name: string; description: string; priceHint: string; icon: string };
  ...
  async function reserveGift(event: FormEvent<HTMLFormElement>) {
    ...
    try {
      const response = await fetch("/api/gifts", {
        ...
        body: JSON.stringify({ giftId: selected.id, name, email }),
      });
      ...
      setName("");
      setEmail("");
      toast.success("Presente reservado! A Liene só descobrirá no dia 11.");
    } catch (error) {
  ...
  return (
    <>
      <div className="mb-7 flex flex-col gap-3 sm:flex-row sm:items-end sm:justify-between">
        <div>
          <p className="mb-2 text-sm font-semibold uppercase tracking-[0.18em] text-[#9b722c]">Escolha com carinho</p>
          <h2 className="font-serif text-3xl font-semibold tracking-tight text-[#391a22] sm:text-4xl">Sugestões disponíveis</h2>
        </div>
      ...
      {loading ? (
        ...
        <div className="rounded-[2rem] border border-[#dac9bd] bg-white/80 px-6 py-14 text-center shadow-sm">
          ...
          <p className="mx-auto mt-2 max-w-xl text-base leading-7 text-[#725f63]">Que alegria! Se quiser, você ainda pode preparar uma mensagem carinhosa para a Liene.</p>
        </div>
      ) : (
        <div className="grid gap-4 sm:grid-cols-2 lg:grid-cols-3">
          {gifts.map((gift, index) => {
            ...
            return (
              <article key={gift.id} className="group flex min-h-64 flex-col rounded-[1.6rem] border border-[#dfd0c6] bg-white/90 p-5 shadow-[0_10px_35px_rgba(74,35,41,0.06)] transition duration-300 hover:-translate-y-1 hover:border-[#c9a568] hover:shadow-[0_18px_45px_rgba(74,35,41,0.12)]">
      ...
      <Dialog open={selected !== null} onOpenChange={(open) => !open && setSelected(null)}>
        <DialogContent className="rounded-[1.5rem] border-[#dec9bd] bg-[#fffdf9] p-6 sm:p-7">
          <DialogHeader>
            ...
            <DialogDescription className="text-base leading-6 text-[#725f63]">Deixe seus dados para confirmar. Assim que reservar, este item sairá da lista para os próximos convidados.</DialogDescription>
          </DialogHeader>
          <form onSubmit={reserveGift} className="mt-2 space-y-4">
            ...
            <div className="space-y-2">
              <label htmlFor="guest-email" className="text-sm font-semibold text-[#4d3036]">Seu e-mail</label>
              <Input id="guest-email" type="email" value={email} onChange={(event) => setEmail(event.target.value)} maxLength={160} autoComplete="email" placeholder="voce@exemplo.com" required className="h-11 rounded-xl border-[#d8c5b8] bg-white" />
            </div>
            <p className="text-sm leading-5 text-[#806e72]">Seu e-mail ficará visível somente para o organizador e será usado apenas se for necessário confirmar algum detalhe.</p>
